...
function renderFit(prod, el){
  ...
  // modo de atuação é EXCLUSIVO do PM&GO integrado — só existe quando a oferta é PM&GO
  const isPmgo = rec==='PM&GO';
  const modo = isPmgo ? (avgAll>=p.cortes.modoDiretivo?2:(avgAll>=p.cortes.modoControle?1:0)) : null;
  store.pmgo={tipo:'fit',rec,modo,avgA:+avgA.toFixed(2),avgB:+avgB.toFixed(2),avgC:+avgC.toFixed(2),answers:vals.slice()};
  const modoHTML = isPmgo
    ? '<h4 style="font-family:Fraunces;color:var(--navy);margin:10px 0 4px">Modo de atuação recomendado <span style="font-size:11px;color:var(--gray);font-weight:400">— intensidade da Obliant no PM&GO</span></h4>'+
...
function dossieFront(c,p){
  ...
  return '<div class="dfront"><div class="df-sig">'+P.sigla+' · fit</div>'+
    ...
    (dd.modo!=null ? '<div class="df-lvl">Modo '+DATA.modos[dd.modo].nome+'</div>' : '')+
